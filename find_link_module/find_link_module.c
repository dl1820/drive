...
typedef struct _latlon_list{
	char *lat;
	char *lon;
	char *info;
	struct latlon_list *next;
} latlon_list;

void finish_with_error(MYSQL *conn);
now_location gps(float lat, float lon, int hd);
data mysql_select(MYSQL *, char query[]);
latlon_list* AppendList(latlon_list* p);
int DisplayList(latlon_list* p);
void FreeMallocList(latlon_list* p);

int main(void){
	char *latlon;
	latlon_list* head = (latlon_list*)malloc(sizeof(latlon_list));
	latlon_list* tail;
	now_location nl = gps(35.838223, 128.682269, 50);
	tail = head;
	head->next = NULL;
	latlon = strtok(nl.curb, "/");
	while( latlon != NULL){	
		tail->lat = latlon;
		latlon = strtok(NULL, "/");	
		tail->lon = latlon;
	...
	struct latlon_list *curr = head->next;
	return 0;
}
...
//			printf("%d\n", tri[tri_index]);
	...
	if ( hd_cnt == 0 ){
		sprintf(query4, "%s'%s'", query4, tonode[0]);
	}
...
latlon_list* AppendList(latlon_list* p){
	p->next = (latlon_list*)malloc(sizeof(latlon_list));
	p = p->next;
	p->next = NULL;
	return p;
}
